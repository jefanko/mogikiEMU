...
void CPU6502::write(uint16_t addr, uint8_t data) { bus->write(addr, data); }

// Helper functions for flags
uint8_t CPU6502::GetFlag(FLAGS6502 f) { return ((st & f) > 0) ? 1 : 0; }
...
uint8_t CPU6502::ABX()
{
    ...
    if ((addr_abs & 0xFF00) != (hi << 8))
        return 1; // Page boundary crossed
    return 0;
}
...
// Instruction: Clear Interrupt Disable Flag
uint8_t CPU6502::CLI()
{
    SetFlag(I, false);
    return 0;
...
// Instruction: Push Processor Status (Flags)
uint8_t CPU6502::PHP()
{
...
// Instruction: Pull Processor Status (Flags)
uint8_t CPU6502::PLP()
{
    sp++;
    st = read(0x0100 + sp);
